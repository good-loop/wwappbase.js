...
(function (window) {
	...
	if (typeof assert === 'undefined') {
		...
		assert = function assert(betrue, msg) {
 ...
 */
	Login.getUser = function (service) {
		...
		try {
			for (var _iterator2 = Login.aliases[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
  ...
  */
	Login.getEmail = function () {
		var emailXId = Login.getId('email');
		if (emailXId) {
			var i = emailXId.lastIndexOf('@');
 ...
 @return: promise */
	...
	var setStateFromServerResponse = function setStateFromServerResponse(res) {
		...
		var newuser = res.cargo && res.cargo.user;
		// {User[]}
		var newaliases = res.cargo && res.cargo.aliases && res.cargo.aliases.slice();
		// check the cookies (which may have changed)
		var cuxid = Cookies.get(COOKIE_UXID);
		var cuserjson = cuxid ? window.localStorage.getItem(cuxid) : null;
		// string[] XIds
		var cookieAliases = [];
		...
		for (var c in cookies) {
			...
			if (c.substr(0, COOKIE_WEBTOKEN.length) === COOKIE_WEBTOKEN) {
				...
				try {
					var cxid = c.substr(COOKIE_WEBTOKEN.length + 1);
		...
		try {
			var _loop = function _loop() {
				var cxid = _step3.value;

				assert(getService(cxid), cxid);

				newaliases.forEach(function (na) {
					if (na.xid === cxid) skip = true;
				});
				if (skip) return "continue";
				newaliases.push({ xid: cxid });
			};

			for (var _iterator3 = cookieAliases[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
				var skip;
	...
	var getService = function getService(xid) {
  ...
  */
	var setUser = function setUser(newuser, newaliases) {
	...
	Login.login = function (person, password) {
		...
		auth = auth.then(setStateFromServerResponse).fail(function (res) {
 ...
 */
	Login.auth = function (service, appId, permissions) {
		...
		window.location = Login.ENDPOINT + "?action=get-auth&app=" + escape(Login.app) + "&appId=" + escape(appId) + "&service=" + service + (permissions ? "&permissions=" + escape(permissions) : '') + "&link=" + (Login.redirectOnLogin || '');
	};
	...
	var doFBLogin_connected = function doFBLogin_connected(response) {
		...
		FB.api('/me?fields=name,about,cover,age_range,birthday,email,gender,relationship_status,website', function (meResponse) {
			...
			aget(Login.ENDPOINT, updateInfo).then( // JWT from YA has to be stored
 ...
 */
  ...
  */
	Login.reset = function (email) {
		...
		var request = aget(Login.ENDPOINT, params).then(function (res) {
			if (res.errors && res.errors.length) {
				// stash the error for showing to the user
				console.error("#login.state", res.errors[0]);
				Login.error = res.errors[0];
  ...
  */
	...
	var aget = function aget(url, data, type) {
  ...
  * @param {*} value 
  */
	var dataPut = function dataPut(formData, key, value) {
