...
if (typeof assert === 'undefined') {
	var assert = function assert(ok, msg) {
		if (ok) return;
		console.error(msg);
		throw new Error(msg);
	};
...
wwutils.setHash = function (unescapedHash) {
	...
	if (history && history.pushState) {
		...
		fireHashChangeEvent({ oldURL: oldURL });
	} else {
...
wwutils.modifyHash = function (newpath, newparams, returnOnly) {
	var _wwutils$parseHash = wwutils.parseHash(),
	    path = _wwutils$parseHash.path,
	    params = _wwutils$parseHash.params;

	var allparams = params || {};
	allparams = Object.assign(allparams, newparams);
	if (!newpath) newpath = path || [];
	var hash = encURI(newpath.join('/'));
	if (wwutils.yessy(allparams)) {
		var kvs = wwutils.mapkv(allparams, function (k, v) {
			return encURI(k) + "=" + (v === null || v === undefined ? '' : encURI(v));
		});
		hash += "?" + kvs.join('&');
	}
	if (returnOnly) {
...
var fireHashChangeEvent = function fireHashChangeEvent(_ref) {
 ...
 */
wwutils.addScript = function (src, _ref2) {
...
*/
var yessy = function yessy(val) {
	if (!val) return false;
	if (typeof val === 'number' || typeof val === 'boolean') {
		return true;
	}
	if ((typeof val === 'undefined' ? 'undefined' : _typeof(val)) === 'object' && val.length === undefined) {
...
wwutils.getStackTrace = function () {
	try {
		...
		var stacktrace = ("" + stack).replace(/\s+/g, ' ').substr(16);
		return stacktrace;
	} catch (error) {
		// oh well
		return "";
...
wwutils.endsWith = function (s, ending) {
	...
	var end = s.substring(s.length - ending.length, s.length);
	return end === ending;
};
 ...
 */
var asPromise = function asPromise(x) {
 ...
 */
var blockProp = function blockProp(obj, propName, message) {
	...
	if (obj[propName] !== undefined) {
		...
		var ex = new Error("Having " + propName + " is blocked! " + message);
	...
	Object.defineProperty(obj, propName, {
		get: function get() {
			var ex = new Error(propName + " is blocked! " + message);
			if (!bphush) console.error(ex, this); // react can swallow stuff
			throw ex;
		},
		set: function set() {
			var ex = new Error("Set " + propName + " is blocked! " + message);
			if (!bphush) console.error(ex, this); // react can swallow stuff
			throw ex;
		}
	});
...
*/
var encURI = function encURI(urlPart) {
...
wwutils.decURI = function (urlPart) {
	var decoded = decodeURIComponent(urlPart);
	return decoded;
};
...
wwutils.clean = function (html, permittedTags) {
	return clean2(html, permittedTags, true);
};
...
**/
wwutils.sanitiseHtml = function (html, permittedTags) {
	return clean2(html, permittedTags, false);
};
