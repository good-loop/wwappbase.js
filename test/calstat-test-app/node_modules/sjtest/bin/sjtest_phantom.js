...
SJTest4Phantom.goPhantom = function () {
	...
	SJTest.waitFor(SJTest4Phantom.isDoneTopLevel, function () {
		var p = SJTest4Phantom.passed.length,
		    f = SJTest4Phantom.failed.length,
		    s = SJTest4Phantom.skipped.length;
		console.log("");
	...
	for (var i = 0; i < SJTest4Phantom._pagesInProcessing.length; i++) {
		...
		var done = page.evaluate(function () {
			try {
				...
				if (new Date().getTime() - window._SJTestStart < 10000) {
					// upto 10 seconds to handle setup messiness
					return false;
				}
