...
public class ICalWriter {
	...
	public void addEvent(ICalEvent event) {
		String s = format(event.start);
		String e = format(event.end);
		
		String _summary = formatText(event.summary);
		
		// Break into 70 char lines
		StringBuilder sum2 = new StringBuilder(_summary.length()+5);
		int i=0;
		for(char c : _summary.toCharArray()) {
			sum2.append(c);
			i++;
			if (i%70 == 0) {
				sum2.append("\r\n ");
...
//		   processing the content type.
		...
		ical.append( // TODO use ICalEvent toString() instead
				"BEGIN:VEVENT\r\n"
...
//	       19980119T070000Z
	...
	public static SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd'T'HHmmss'Z'");	
