...
Misc.Logo = function (_ref5) {
	...
	if (service === _C2.default.app.service) {
		file = _C2.default.app.logo;
...
Misc.SavePublishDiscard = function (_ref20) {
	...
	(0, _sjtest.assert)(_C2.default.TYPES.has(type), 'Misc.SavePublishDiscard');
	(0, _sjtest.assMatch)(id, String);
	var localStatus = _DataStore2.default.getLocalEditsStatus(type, id);
	var isSaving = _C2.default.STATUS.issaving(localStatus);
	var item = _DataStore2.default.getData(type, id);
	// request a save?
	if (_C2.default.STATUS.isdirty(localStatus) && !isSaving) {
	...
	var noEdits = item && _C2.default.KStatus.isPUBLISHED(item.status) && _C2.default.STATUS.isclean(localStatus) && !item.modified;
	...
	return _react2.default.createElement(
		...
		_react2.default.createElement(
			'button',
			{ className: 'btn btn-default', disabled: isSaving || _C2.default.STATUS.isclean(localStatus), onClick: function onClick() {
...
Misc.SubmitButton = function (_ref21) {
	...
	var doSubmit = function doSubmit(e) {
		_DataStore2.default.setValue(tpath, _C2.default.STATUS.saving);
		_ServerIO2.default.load(url, params).then(function (res) {
			_DataStore2.default.setValue(tpath, _C2.default.STATUS.clean);
		}, function (err) {
			_DataStore2.default.setValue(tpath, _C2.default.STATUS.dirty);
		});
	};

	var localStatus = _DataStore2.default.getValue(tpath);
	// show the success message instead?
	if (onSuccess && _C2.default.STATUS.isclean(localStatus)) {
		return onSuccess;
	}
	var isSaving = _C2.default.STATUS.issaving(localStatus);
	var vis = { visibility: isSaving ? 'visible' : 'hidden' };
